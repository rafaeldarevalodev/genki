...
Supports VoxMLX (V1) and MFA (V2) evaluators.

Usage:
    python main_api.py          # Run with default (VoxMLX)
    python main_api.py --port 10301  # Custom port
    python main_api.py --eval mfa    # Use MFA evaluator
"""
...
async def list_evaluators():
    ...
    return {
        ...
        "default": EvaluatorFactory._default.value,
    }
...
async def evaluate(
    ...
    evaluator: str = Form("voxmlx", description="Evaluator type"),
    ...
    Args:
        ...
        evaluator: Evaluator type ("voxmlx" or "mfa")
    ...
    try:
        eval_type = EvaluatorType(evaluator)
    except ValueError:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid evaluator: {evaluator}. "
            f"Available: {EvaluatorFactory.available_evaluators()}"
        )
    
    # Get evaluator
    evaluator_impl = get_evaluator(eval_type)
    
    # Check health
    if not await evaluator_impl.health_check():
        raise HTTPException(
            status_code=503,
            detail=f"Evaluator {evaluator} is not available. "
...
def main():
    ...
    parser.add_argument(
        "--port", type=int, default=10301,
        help="Port to bind to"
    )
    parser.add_argument(
        "--eval", type=str, default="voxmlx",
        choices=["voxmlx", "mfa"],
        help="Default evaluator"
    )
    parser.add_argument(
    ...
    EvaluatorFactory.set_default(EvaluatorType(args.eval))
    
    logger.info(f"Starting on {args.host}:{args.port}")
    logger.info(f"Default evaluator: {args.eval}")
